...
[NOTE] Until this [bug](https://gitlab.kitware.com/cmake/cmake/issues/19973) is not resolved by CMake contributor you should manually rename the result *.dmg file to name pattern which contains app version EkroneWallet-{VERSION}.dmg where VERSION defined as UN_VERSION inside the EkroneWallet.cmake file
...
Clone the source code to some path [EXAMPLE: C:\Ekrone\SOURCE\ekronewallet] and then run "x64 Native Tools Command Prompt for VS 2019" and type the lines below step by step [make sure you have properly replaced the paths for your system]:

```
cd C:\Ekrone\SOURCE\ekronewallet
...
# fetch sources
git clone https://github.com/ekronemaster/Ekronewallet.git
cd EkroneWallet-infinity
git submodule init
git submodule update
...
```
cd Release
windeployqt Ekrone.exe
cd ..
cpack -C Release

```

This will generate the EkroneWallet installation package inside the build directory. [Example: ```Ekrone-1.0.2-beta-win64.exe```]
...
Alternatively you can use the Visual Studio GUI for building. To do so, run just the first cmake command from the command list above ["%CMAKE_PATH%"\cmake.exe] with option ```-G "Visual Studio 15 2017 Win64"``` instead of ```-G "NMake Makefiles```", then open the generated file `build/release/Ekrone.sln` in Visual Studio. Select build type 'Release' and build the target 'EkroneWallet' or for a distribution package build target 'PACKAGE'.

When compiling from Visual Studio you need to add /bigobj switch flag in EkroneWallet project, C/C++->Command Line
